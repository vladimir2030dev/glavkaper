...
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields = array(
		'author' => sprintf(
			'<p class="comment__label__form">%s %s</p>',
			sprintf(
	...
	if ( has_action( 'set_comment_cookies', 'wp_set_comment_cookies' ) && get_option( 'show_comments_cookies_opt_in' ) ) {
		...
		$fields['cookies'] = sprintf(
			...
			sprintf(
				...
				__( 'Нажимая кнопку отправить, вы соглашаетесь с <a class="form__link" href="<?php echo get_home_url(); ?>" target="_blank">
				действующей политикой конфиденциальности
			</a>' )
			)
		);
	...
	$defaults = array(
		...
		'comment_field'        => sprintf(
			'<p class="comment__label__form">%s %s</p>',
			sprintf(
				'<label for="comment"  class="form-label">%s</label>',
				_x( 'Comment', 'noun' )
			),
		...
		'title_reply_before'   => '<h3 id="reply-title" class="comment__title">',
		'title_reply_after'    => '</h3>',
		'cancel_reply_before'  => ' <small>',
		'cancel_reply_after'   => '</small>',
		'cancel_reply_link'    => __( 'Cancel reply' ),
		'label_submit'         => __( 'Post Comment' ),
		'submit_button'        => '<input name="%1$s" type="submit" class="btn btn-primary" id="%2$s" value="%4$s" />',
		'submit_field'         => '<p class="btn btn-primary">%1$s %2$s</p>',
		'format'               => 'xhtml',
	);
